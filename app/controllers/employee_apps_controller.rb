class EmployeeAppsController < ApplicationController
  before_action :set_employee_app, only: %i[ show edit update destroy ]

  # GET /employee_apps or /employee_apps.json
  def index
    @employee_apps = EmployeeApp.all
  end

  # GET /employee_apps/1 or /employee_apps/1.json
  def show
  end
  ...
  # POST /employee_apps or /employee_apps.json
  def create
    @employee_app = EmployeeApp.new(employee_app_params)

    respond_to do |format|
      if @employee_app.save
        format.html { redirect_to employee_app_url(@employee_app), notice: "Employee app was successfully created." }
  ...
  # PATCH/PUT /employee_apps/1 or /employee_apps/1.json
  def update
    respond_to do |format|
      if @employee_app.update(employee_app_params)
        format.html { redirect_to employee_app_url(@employee_app), notice: "Employee app was successfully updated." }
  ...
  # DELETE /employee_apps/1 or /employee_apps/1.json
  def destroy
